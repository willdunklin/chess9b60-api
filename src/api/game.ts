import { getGame, updatePlayer, dbclient } from "../db";
import { PutItemCommand, AttributeValue } from "@aws-sdk/client-dynamodb";
...
async function assignPlayerID(id: string, token: string, game: {[key: string]: AttributeValue} | undefined) {
    ...
    if(white !== "") {
        if(await updatePlayer(dbclient, "bgio", id, token, true) !== token)  // make the player black
            return null; // if the player is already taken, make them spectator
        player = "1";

    } else if (black !== "") {
        if(await updatePlayer(dbclient, "bgio", id, token, false) !== token) // make the player white
            return null; // if the player is already taken, make them spectator
        player = "0";
    } else {
        return "invalid";
